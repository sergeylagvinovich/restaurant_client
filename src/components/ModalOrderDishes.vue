<template>
  <div class="modal-mask">
    <div class="modal-wrapper">
      <div class="modal-container">
        <div class="modal-header">
          <slot name="header" class="title-h4">
            <div class="user-name title-h4">Order details</div>
          </slot>
        </div>
        <div class="modal-body">
          <slot name="body">
            <div class="check-order">
              <div class="frame" v-for="item in dishes">
                <div class="center">
                  <section class="todo-cmp">
                    <header class="todo-cmp__header">
                      <div class="title-h6 order-filter title-thin">{{item.name}}</div>
                      <div class="menu-item-image">
                        <img class="menu-img" :src="getPath+item.imgPath" alt="">
                      </div>
                      <div class="title-h5 order-name">Count {{item.count}}</div>
                    </header>
                  </section>
                </div>
              </div>
            </div>
          </slot>
        </div>

        <div class="modal-footer">
          <slot name="footer">
            <button class="button button-size-small button-style-outline button-complete" @click="$emit('close')">Close</button>
            <button class="button button-size-small button-style-outline button-complete" @click="download">Download file</button>
          </slot>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import publicPaths from "@/store/publicPaths";
import instance from "@/store/axios";
export default {
  name: "ModalOrderDishes",
  props:{
    dishes:[],
    filename:null,
  },
  computed:{
    getPath(){
      return publicPaths.images;
    },
  },
  methods:{
    download(){
      window.open(`http://localhost:8080/api/v1/orders/download/${this.filename}`, '_blank');
    }
  }
}
</script>

<style scoped>
.modal-mask {
  position: fixed;
  z-index: 9998;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background-color: rgba(0, 0, 0, 0.5);
  display: table;
  transition: opacity 0.3s ease;
}

.modal-wrapper {
  display: table-cell;
  vertical-align: middle;
}

.modal-container {
  width: 80%;
  height: 800px;
  overflow: auto;
  margin: 0px auto;
  padding: 20px 30px;
  background-color: #fff;
  border-radius: 2px;
  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.33);
  transition: all 0.3s ease;
  font-family: Helvetica, Arial, sans-serif;
}

.modal-header h3 {
  margin-top: 0;
  color: #42b983;
}

.modal-body {
  margin: 20px 0;
}

.modal-default-button {
  float: right;
}

/*
 * The following styles are auto-applied to elements with
 * transition="modal" when their visibility is toggled
 * by Vue.js.
 *
 * You can easily play with the modal transition by editing
 * these styles.
 */

.modal-enter {
  opacity: 0;
}

.modal-leave-active {
  opacity: 0;
}

.modal-enter .modal-container,
.modal-leave-active .modal-container {
  -webkit-transform: scale(1.1);
  transform: scale(1.1);
}
</style>